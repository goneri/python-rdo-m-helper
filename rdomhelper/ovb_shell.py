...
from rdomhelper import ovbundercloud
from rdomhelper import virtbmc
...
def deploy_bm(nova_api, neutron, provisioner, instance_name, image_name, ip, flavor='m1.small'):
    group = os_provisioner.get_provision_security_group(nova_api)
...
def configure_undercloud(undercloud, repositories, guest_image_path, guest_image_checksum, neutron):
...
def cli(os_auth_url, os_username, os_password, os_tenant_name, host0_ip, undercloud_ip, config_file):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for bm_node in bm_nodes:
            ...
            instackenv.append({
                ...
                # TODO: We should get these informations from the baremetal node's flavor
                "cpu": "4",
                "memory": "8196",
                "disk": "80",
                "arch": "x86_64",
    ...
    vm_undercloud = ovbundercloud.OVBUndercloud(
        nova_api=nova_api,
        neutron=neutron,
        provisioner=config['provisioner'],
        key_filename=config['ssh']['private_key'],
    ...
    vm_undercloud.create_file('instackenv.json', json.dumps(instackenv), user='stack')
    undercloud_conf = """
[DEFAULT]
...
[auth]
"""
    vm_undercloud.create_file('undercloud.conf', undercloud_conf.format(undercloud_ip=undercloud_ip), user='stack')
    configure_undercloud(
        ...
        neutron)
    vm_undercloud.enable_neutron_hack(os_username, os_password, os_tenant_name, os_auth_url)
    set_dhcp_parameter(neutron, filename='inspector.ipxe')
    vm_undercloud.start_overcloud_inspector()
